...
public class NewsTypeAction extends BaseController{
	...
	public String orderNum() throws Exception {
		...
		Integer dbid = ParamUtil.getIntParam(request, "dbid", -1);
		if (dbid > 0) {
			List<NewsType> newsTypes = newsTypeManageImpl.find("from NewsType where parent.dbid=? order by orderNum", new  Object[]{dbid});
	...
	public void getNewsTypeByDbid() throws Exception {
		...
		if(dbid>0){
			...
			if(null!=newsType2){
				...
				object.put("orderNum", newsType2.getOrderNum());
